...
```
```{r}
elev_df <- as.data.frame(dem_port, xy = TRUE)
slope_df <- as.data.frame(port_slope, xy = TRUE)
...
port_lc_14_road <- port_14_10_rc["road"]
plot(port_lc_14_road)
```
```{r}
# Calculate the mean value for a 15x15 cell neighborhood
port_14_road_focal = focal2(port_14_10_rc["road"], matrix(1, 15, 15), "mean")
# histogram of the percentage of 1 (impervious) per 150x150m cell
hist(port_14_road_focal$road)

# Plot the result of the mean calculation
# Why do these two methods look different?
plot(port_14_road_focal)
```

## forest
```{r}
# Reclassify the forest
port_14_10_rc <- port_14_10_crop %>% 
  mutate(forest = case_when(
    port_51740_landcover_2014.tif != 3  ~ 0,
    port_51740_landcover_2014.tif == 3 ~ 1
  ))
port_18_10_rc <- port_18_10_crop %>% 
  mutate(forest = case_when(
    port_51740_landcover_2018.tif != 3  ~ 0,
    port_51740_landcover_2018.tif == 3 ~ 1
  ))
# Plot the reclassified raster image
port_lc_14_forest <- port_14_10_rc["forest"]
plot(port_lc_14_forest)
port_lc_18_forest <- port_18_10_rc["forest"]
plot(port_lc_18_forest)

```
```{r}
port_change <- port_14_10_rc["forest"]- port_18_10_rc["forest"]
port_change_rc <- port_change %>% 
  mutate(forest_lost = case_when(
    forest <= 0  ~ 0,
    forest > 0 ~ 1
  ))
port_forest_lost<-port_change_rc["forest_lost"]
plot(port_forest_lost)
```
```{r}
# Calculate the mean value for a 10x10 cell neighborhood
#port_forest_lost_focal = focal2(port_forest_lost, matrix(1, 10, 10), "mean")
# histogram of the percentage of 1 (forest lost) per 30x30m cell
#hist(port_forest_lost_focal)

# Plot the result of the mean calculation
#plot(port_forest_lost_focal)
```

## Wetland
```{r}
# Reclassify the wetland
port_14_10_rc <- port_14_10_crop %>% 
  mutate(wetland = case_when(
    port_51740_landcover_2014.tif != 2  ~ 0,
    port_51740_landcover_2014.tif == 2 ~ 1
  ))
port_18_10_rc <- port_18_10_crop %>% 
  mutate(wetland = case_when(
    port_51740_landcover_2018.tif != 2  ~ 0,
    port_51740_landcover_2018.tif == 2 ~ 1
  ))
# Plot the reclassified raster image
port_lc_14_wetland <- port_14_10_rc["wetland"]
plot(port_lc_14_wetland)
port_lc_18_wetland <- port_18_10_rc["wetland"]
plot(port_lc_18_wetland)

```
```{r}
# Calculate the mean value for a 15x15 cell neighborhood
port_14_wetland_focal = focal2(port_14_10_rc["wetland"], matrix(1, 10, 10), "mean")
# histogram of the percentage of 1 (impervious) per 150x150m cell
hist(port_14_wetland_focal$wetland)

# Plot the result of the mean calculation
# Why do these two methods look different?
plot(port_14_wetland_focal)
```
...
isle_tracts14 <- 
  ...
  st_transform(st_crs(isle_14_10)) %>%
  rename(TotalPop = B01003_001E, 
         Whites = B02001_002E,
         MedHHInc = B19013_001E,
         TotalUnit = B25002_001E,
...
isle_tracts18 <- 
  ...
  st_transform(st_crs(isle_18_10)) %>%
  rename(TotalPop = B01003_001E, 
         Whites = B02001_002E,
         MedHHInc = B19013_001E,
         TotalUnit = B25002_001E,
...
```{r}
port_block <- rbind(port_tracts14,port_tracts18)
```

```{r}
# extract points
#pnt = st_sample(st_as_sfc(st_bbox(port_14_pcnt_imperv)), 1000)

#port_imprevious_pt14 <- st_extract(port_14_pcnt_imperv, pnt) %>% 
 # st_as_sf()%>%
#  st_transform(st_crs(isle_14_10))
```
...
```{r}
#extract to census block
Port14_mean_imperv <- aggregate(!is.na(port_14_10_rc), port_tracts14, FUN = mean) %>% 
  st_as_sf()%>% 
  mutate (year = "2014")
```

```{r}
Port18_mean_imperv <- aggregate(!is.na(port_18_10_rc), port_tracts18, FUN = mean) %>% 
  st_as_sf()%>% 
  mutate (year = "2018")
```

```{r}
port14_imperv_block <- Port14_mean_imperv  %>% 
filter(lc == 1) 

port18_imperv_block <- Port18_mean_imperv %>% 
filter(lc == 1)
...
```{r}
ggplot() +
  geom_sf(data=port_block , aes(fill = q5(TotalPop) ),color ="grey80") +
   facet_wrap(~year) +
   geom_sf(data=st_union(port18_imperv_block),fill = "transparent", color = "grey10",size= 1)+
  scale_fill_manual(values = palette5,
                    labels = qBr(port_block , "TotalPop"),
                    name = "Total Population\n(Quintile Breaks)")+
   labs(title = "Total Population in Impervious Block Groups", 
        subtitle = "Portsmouth; 2014-2018") +
  mapTheme()
```
```{r}
ggplot() +
  geom_sf(data=port_block , aes(fill = q5(pctWhite) ),color ="grey80") +
   facet_wrap(~year) +
   geom_sf(data=st_union(port18_imperv_block),fill = "transparent", color = "grey10",size= 1)+
  scale_fill_manual(values = palette5,
                    labels = qBr(port_block , "pctWhite"),
                    name = "Percentage of White\n(Quintile Breaks)")+
   labs(title = "Percentage of White in Impervious Block Groups", 
        subtitle = "Portsmouth; 2014-2018") +
  mapTheme()
```
```{r}
ggplot() +
  geom_sf(data=port_block , aes(fill = q5(MedHHInc) ),color ="grey80") +
   facet_wrap(~year) +
   geom_sf(data=st_union(port18_imperv_block),fill = "transparent", color = "grey10",size= 1)+
  scale_fill_manual(values = palette5,
                    labels = qBr(port_block , "MedHHInc"),
                    name = "Medium Income\n(Quintile Breaks)")+
   labs(title = "Medium Income in Impervious Block Groups", 
        subtitle = "Portsmouth; 2014-2018") +
  mapTheme()
```
...
```{r}
ggplot() +
  geom_sf(data=port_block , aes(fill = q5(TotalUnit) ),color ="grey80") +
   facet_wrap(~year) +
   geom_sf(data=st_union(port18_imperv_block),fill = "transparent", color = "grey10",size= 1)+
  scale_fill_manual(values = palette5,
                    labels = qBr(port_block , "TotalUnit"),
                    name = "Housing units\n(Quintile Breaks)")+
   labs(title = "Housing units in Impervious Block Groups", 
        subtitle = "Portsmouth; 2014-2018") +
  mapTheme()
```
...
```{r}
Port14_mean_imperv <- Port14_mean_imperv[c(2,3,4)]
Port18_mean_imperv <- Port18_mean_imperv[c(2,3,4)]
imperv_all <- rbind(Port14_mean_imperv, Port18_mean_imperv)

port_block_all <- cbind(port_block,imperv_all) %>%
  mutate(impervious = ifelse(lc == 1, "impervious", "pervious"),
         ) 
...
```{r}
port_block.Summary <- 
  st_drop_geometry(port_block_all) %>%
  group_by(year, impervious) %>%
  drop_na() %>%
  summarize(Income = mean(MedHHInc, na.rm = T),
            Population = mean(TotalPop, na.rm = T),
            Percent_White = mean(pctWhite, na.rm = T),
            Housing_Units = mean(TotalUnit, na.rm = T))

port_block.Summary %>%
  gather(Variable, Value, -year, -impervious) %>%
  ggplot(aes(year, Value, fill = impervious)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~Variable, scales = "free", ncol=5) +
  scale_fill_manual(values = c("#bae4bc", "#0868ac")) +
  labs(title = "Indicator differences across time and space", caption = "Figure 9") +
...
```{r}
james_block <- rbind(james_tracts14,james_tracts18)
```

```{r}
#extract to census block
james14_mean_imperv <- aggregate(!is.na(james_14_10_rc), james_tracts14, FUN = mean) %>% 
  st_as_sf()%>% 
  mutate (year = "2014")
james18_mean_imperv <- aggregate(!is.na(james_18_10_rc), james_tracts18, FUN = mean) %>% 
  st_as_sf()%>% 
  mutate (year = "2018")
```

```{r}
james14_imperv_block <- james14_mean_imperv  %>% 
filter(lc == 1) 

james18_imperv_block <- james18_mean_imperv %>% 
filter(lc == 1)
...
```{r}
ggplot() +
  geom_sf(data=james_block , aes(fill = q5(TotalPop) ),color ="grey80") +
   facet_wrap(~year) +
   geom_sf(data=st_union(james18_imperv_block),fill = "transparent", color = "grey10",size= 1)+
  scale_fill_manual(values = palette5,
                    labels = qBr(james_block , "TotalPop"),
                    name = "Total Population\n(Quintile Breaks)")+
   labs(title = "Total Population in Impervious Block Groups", 
        subtitle = "James City; 2014-2018") +
  mapTheme()

ggplot() +
  geom_sf(data=james_block , aes(fill = q5(pctWhite) ),color ="grey80") +
   facet_wrap(~year) +
   geom_sf(data=st_union(james18_imperv_block),fill = "transparent", color = "grey10",size= 1)+
  scale_fill_manual(values = palette5,
                    labels = qBr(james_block , "pctWhite"),
                    name = "Percentage of White\n(Quintile Breaks)")+
   labs(title = "Percentage of White in Impervious Block Groups", 
        subtitle = "James City; 2014-2018") +
  mapTheme()
...
ggplot() +
  geom_sf(data=james_block , aes(fill = q5(MedHHInc) ),color ="grey80") +
   facet_wrap(~year) +
   geom_sf(data=st_union(james18_imperv_block),fill = "transparent", color = "grey10",size= 1)+
  scale_fill_manual(values = palette5,
                    labels = qBr(james_block , "MedHHInc"),
                    name = "Medium Income\n(Quintile Breaks)")+
   labs(title = "Medium Income in Impervious Block Groups", 
        subtitle = "James City; 2014-2018") +
  mapTheme()
...
ggplot() +
  geom_sf(data=james_block , aes(fill = q5(TotalUnit) ),color ="grey80") +
   facet_wrap(~year) +
   geom_sf(data=st_union(james18_imperv_block),fill = "transparent", color = "grey10",size= 1)+
  scale_fill_manual(values = palette5,
                    labels = qBr(james_block , "TotalUnit"),
                    name = "Housing units\n(Quintile Breaks)")+
   labs(title = "Housing units in Impervious Block Groups", 
        subtitle = "James City; 2014-2018") +
  mapTheme()
...
```{r}
james14_mean_imperv <- james14_mean_imperv[c(2,3,4)]
james18_mean_imperv <- james18_mean_imperv[c(2,3,4)]
imperv_all <- rbind(james14_mean_imperv, james18_mean_imperv)

james_block_all <- cbind(james_block,imperv_all) %>%
  mutate(impervious = ifelse(lc == 1, "impervious", "pervious"),
         ) 
...
```{r}
james_block.Summary <- 
  st_drop_geometry(james_block_all) %>%
  group_by(year, impervious) %>%
  drop_na() %>%
  summarize(Income = mean(MedHHInc, na.rm = T),
            Population = mean(TotalPop, na.rm = T),
...
```{r}
isle_block <- rbind(isle_tracts14,isle_tracts18)
```

```{r}
#extract to census block
isle14_mean_imperv <- aggregate(!is.na(isle_14_10_rc), isle_tracts14, FUN = mean) %>% 
  st_as_sf()%>% 
  mutate (year = "2014")
isle18_mean_imperv <- aggregate(!is.na(isle_18_10_rc), isle_tracts18, FUN = mean) %>% 
  st_as_sf()%>% 
  mutate (year = "2018")
```

```{r}
isle14_imperv_block <- isle14_mean_imperv  %>% 
filter(lc == 1) 

isle18_imperv_block <- isle18_mean_imperv %>% 
filter(lc == 1)
...
```{r}
ggplot() +
  geom_sf(data=isle_block , aes(fill = q5(TotalPop) ),color ="grey80") +
   facet_wrap(~year) +
   geom_sf(data=st_union(isle18_imperv_block),fill = "transparent", color = "grey10",size= 1)+
  scale_fill_manual(values = palette5,
                    labels = qBr(isle_block , "TotalPop"),
                    name = "Total Population\n(Quintile Breaks)")+
   labs(title = "Total Population in Impervious Block Groups", 
        subtitle = "Isle of Wight; 2014-2018") +
  mapTheme()

ggplot() +
  geom_sf(data=isle_block , aes(fill = q5(pctWhite) ),color ="grey80") +
   facet_wrap(~year) +
   geom_sf(data=st_union(isle18_imperv_block),fill = "transparent", color = "grey10",size= 1)+
  scale_fill_manual(values = palette5,
                    labels = qBr(isle_block , "pctWhite"),
                    name = "Percentage of White\n(Quintile Breaks)")+
   labs(title = "Percentage of White in Impervious Block Groups", 
        subtitle = "Isle of Wight; 2014-2018") +
  mapTheme()
...
ggplot() +
  geom_sf(data=isle_block , aes(fill = q5(MedHHInc) ),color ="grey80") +
   facet_wrap(~year) +
   geom_sf(data=st_union(isle18_imperv_block),fill = "transparent", color = "grey10",size= 1)+
  scale_fill_manual(values = palette5,
                    labels = qBr(isle_block , "MedHHInc"),
                    name = "Medium Income\n(Quintile Breaks)")+
   labs(title = "Medium Income in Impervious Block Groups", 
        subtitle = "Isle of Wight; 2014-2018") +
  mapTheme()
...
ggplot() +
  geom_sf(data=isle_block , aes(fill = q5(TotalUnit) ),color ="grey80") +
   facet_wrap(~year) +
   geom_sf(data=st_union(isle18_imperv_block),fill = "transparent", color = "grey10",size= 1)+
  scale_fill_manual(values = palette5,
                    labels = qBr(isle_block , "TotalUnit"),
                    name = "Housing units\n(Quintile Breaks)")+
   labs(title = "Housing units in Impervious Block Groups", 
        subtitle = "Isle of Wight; 2014-2018") +
  mapTheme()
...
```{r}
isle14_mean_imperv <- isle14_mean_imperv[c(2,3,4)]
isle18_mean_imperv <- isle18_mean_imperv[c(2,3,4)]
imperv_all <- rbind(isle14_mean_imperv, isle18_mean_imperv)

isle_block_all <- cbind(isle_block,imperv_all) %>%
  mutate(impervious = ifelse(lc == 1, "impervious", "pervious"),
         ) 
...
```{r}
isle_block.Summary <- 
  st_drop_geometry(isle_block_all) %>%
  group_by(year, impervious) %>%
  drop_na() %>%
  summarize(Income = mean(MedHHInc, na.rm = T),
            Population = mean(TotalPop, na.rm = T),
