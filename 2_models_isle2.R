...
isle14_df <- isle14_df %>% mutate(
  lcre = case_when(lcchange == -1 ~ 1,
                   lcchange == 1 ~ 0,
                   lcchange == 0 ~ 0)
)
isle14_df$lcchange <- as.factor(isle14_df$lcchange)
isle14_df$lcre <- as.factor(isle14_df$lcre)
## try multi-class
isle14_sample <- sample_n(isle14_df,500000)

### Initial Split for Training and Test
data_split <- initial_split(isle14_sample, strata = "lcchange", prop = 0.75)



isle_train <- training(data_split)
isle_test  <- testing(data_split)


### Cross Validation
## LOGOCV on Neighborhood with group_vfold_cv()
cv_splits_geo <- group_vfold_cv(isle_train,  
                                group = "GEOID")
print(cv_splits_geo)

# Create recipe
model_rec <- recipe(lcchange ~ ., data = isle_train) %>%
