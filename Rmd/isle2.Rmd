...
# Resample the raster image to 10x10 cellstable
...
isle_18_10_rc <- isle_18_10_crop %>% 
  mutate(
    originallc = islelc_18_10x10.tif,
    lc = case_when(
...
isle_14_canopy<- isle_14_10_crop %>% 
  mutate(canopy = case_when(
    ...
    islelc_14_10x10.tif == 10 ~ 2, #Tree Canopy over Structure
    islelc_14_10x10.tif == 11 ~ 3, #Tree Canopy over Other
    islelc_14_10x10.tif == 12 ~ 4, #Tree Canopy over road
...
isle_18_canopy <- isle_18_10_crop%>% 
  mutate(canopy = case_when(
    ...
    islelc_18_10x10.tif == 10 ~ 2, #Tree Canopy over Structure
    islelc_18_10x10.tif == 11 ~ 3, #Tree Canopy over Other
    islelc_18_10x10.tif == 12 ~ 4, #Tree Canopy over road
...
water_14 <- focal2(isle_14_water["water"], matrix(1, 25, 25), "mean")
water_14 <- focal2(water_14["water"], matrix(1, 25, 25), "mean")
water_14 <- focal2(water_14["water"], matrix(1, 25, 25), "mean")
water_18 <- focal2(isle_14_water["water"], matrix(1, 25, 25), "mean")
water_18 <- focal2(water_18["water"], matrix(1, 25, 25), "mean")
water_18 <- focal2(water_18["water"], matrix(1, 25, 25), "mean")
# rename before join
water_14<-water_14%>%
  rename(pctdistw = water)
water_18<-water_18%>%
  rename(pctdistw = water)
```
...
soilclass <- soil%>%
  mutate(soil = case_when(
    ...
    isle__Ra_1 == 'A/D' ~ 0.4,
    isle__Ra_1 == 'A' ~ 0,
    is.na(isle__Ra_1) ~ mean(isle__Ra_1)
  )) %>%
...
```{r}
rm(dem_isle, isle_14_10_crop, isle_14_10_rc, isle_14_barren, isle_14_canopy, isle_14_other, isle_14_pcnt_imperv, isle_14_permeable,isle_14_road, isle_14_structure, isle_14_water, isle_18_10_crop, isle_18_10_rc, isle_18_barren, isle_18_canopy, isle_18_other, isle_18_pcnt_imperv, isle_18_permeable,isle_18_road, isle_18_structure, isle_18_water)
```
...
isle18_df <- 
  ...
  rename(originallc = islelc_18_10x10.tif ,
          terrain = USGS_1_n38w077_20170509.tif)l
islech_df <- 
...
tm_borders(col = 'grey')+
  ...
  tm_legend(outside = TRUE,hist.width = 7,hist.height = 7) +
  tm_shape(isle14_crop[c(4)],legend.only = TRUE) +
  tm_legend(outside = TRUE,hist.width = 7,hist.height = 7) +
  tm_raster(palette = palette10,legend.hist = TRUE)  +
  tm_shape(isle14_crop[c(5)],legend.only = TRUE) +
  tm_legend(outside = TRUE,hist.width = 7,hist.height = 7) +
  tm_raster(palette = palette10,legend.hist = TRUE)  +
   tm_shape(isle14_crop[c(6)],legend.only = TRUE) +
  tm_legend(outside = TRUE,hist.width = 7,hist.height = 7) +
  tm_raster(palette = palette10,legend.hist = TRUE)  +
  tm_shape(isle14_crop['lcp']) +
  tm_raster(palette = palette5)  +
tm_layout(
	inner.margins=c(.1,.1, .1,0.01),
	legend.outside.position	= "right",
